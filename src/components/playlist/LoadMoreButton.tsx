...
export const LoadMoreButton = ({ hasMoreSpins, loadingMore, onLoadMore }: LoadMoreButtonProps) => {
  ...
  return (
    <div className={`flex justify-center ${isEmbedMode ? 'p-4 bg-transparent' : 'mt-4'}`}>
      <Button
        variant="outline"
        onClick={onLoadMore}
        disabled={loadingMore}
        className={`w-full sm:w-auto ${isEmbedMode ? 'border-gray-600 bg-gray-800 text-white hover:bg-gray-700' : ''}`}
      >
